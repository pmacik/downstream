...
git checkout downstream/master openshift # get the downstream updates on top of master
...
# Apply patches
git apply openshift/patches/*.patch
git add *
git commit -m ":fire: Apply carried patches."
